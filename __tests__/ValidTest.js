...
describe("Valid 클래스 테스트", () => {
  describe("validateCarNameIsNotEmpty: 자동차 이름 입력 유효성 검사", () => {
    test("자동차 이름이 빈 배열일 경우 에러", async () => {
      ...
      await expect(valid.validateCarNameIsNotEmpty()).rejects.toThrow("[ERROR] 자동차 이름을 입력하지 않았습니다.");
    });

    test("자동차 이름이 비어있지 않으면 에러 없음", async () => {
      const valid = new Valid(["pobi", "woni"], "5");
      await expect(valid.validateCarNameIsNotEmpty()).resolves.not.toThrow();
    });
  });
  ...
  describe("validateCarNameLength: 자동차 이름 길이 유효성 검사", () => {
    test("자동차 이름이 5자를 초과할 경우 에러", async () => {
      ...
      await expect(valid.validateCarNameLength()).rejects.toThrow("[ERROR] 자동차 이름이 5글자를 초과합니다.");
    });

    test("모든 자동차 이름이 5자 이하일 경우 에러 없음", async () => {
      const valid = new Valid(["pobi", "woni"], "5");
      await expect(valid.validateCarNameLength()).resolves.not.toThrow();
    });
  });
  ...
  describe("validateTryCountIsNotEmpty: 시도 횟수 입력 유효성 검사", () => {
    test("시도 횟수가 빈 문자열일 경우 에러", async () => {
      ...
      await expect(valid.validateTryCountIsNotEmpty()).rejects.toThrow("[ERROR] 시도할 횟수를 입력하지 않았습니다.");
    });

    test("시도 횟수가 비어있지 않으면 에러 없음", async () => {
      const valid = new Valid(["pobi", "woni"], "5");
      await expect(valid.validateTryCountIsNotEmpty()).resolves.not.toThrow();
    });
  });
